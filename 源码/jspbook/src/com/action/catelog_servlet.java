...
public class catelog_servlet extends HttpServlet
{
	...
	public void catelogAdd(HttpServletRequest req,HttpServletResponse res)
	{
		...
		String del="no";
		String sql="insert into t_catelog_pqy(name,jieshao,del) values(?,?,?)";
		Object[] params={name,jieshao,del};
		DB mydb=new DB();
		mydb.doPstm(sql, params);
		mydb.closed();
		
		req.setAttribute("message", "类别添加成功");
	...
	{
		String sql="update t_catelog_pqy set del='yes' where id="+Integer.parseInt(req.getParameter("id"));
		Object[] params={};
		DB mydb=new DB();
		mydb.doPstm(sql, params);
		mydb.closed();
		
		req.setAttribute("message", "类别删除成功");
		req.setAttribute("path", "catelog?type=catelogMana");
